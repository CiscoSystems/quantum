...
""".strip() % (fixed_ips['ip_address'],
    ...
    def test_create_port_with_ipv6_slaac_subnet_in_fixed_ips(self):
        """Test port create with an IPv6 SLAAC subnet incl in fixed IPs."""
        res = self._create_network(fmt=self.fmt, name='net',
                                   admin_state_up=True)
        network = self.deserialize(self.fmt, res)
        subnet = self._make_v6_subnet(network, constants.IPV6_SLAAC)
        # Upon port creation, an address for an IPv6 SLAAC subnets should be
        # automatically generated by RADVD, so there should be no call to
        # the plugin's _generate_ip method.
        with mock.patch.object(db_base_plugin_v2.NeutronDbPluginV2,
                               '_generate_ip') as mock_generate_ip:
            kwargs = {"fixed_ips": [{'subnet_id': subnet['subnet']['id']}]}
            res = self._create_port(self.fmt, net_id=network['network']['id'],
                                    **kwargs)
            port = self.deserialize(self.fmt, res)
            self.assertFalse(mock_generate_ip.called)
            self.assertEqual(1, len(port['port']['fixed_ips']))
            self.assertEqual(self._calc_ipv6_addr_by_EUI64(port, subnet),
                             port['port']['fixed_ips'][0]['ip_address'])

    def test_create_port_with_ipv6_dhcp_stateful_subnet_in_fixed_ips(self):
        """Test port create with an IPv6 DHCP stateful subnet in fixed IPs."""
        res = self._create_network(fmt=self.fmt, name='net',
                                   admin_state_up=True)
        network = self.deserialize(self.fmt, res)
        subnet = self._make_v6_subnet(network, constants.DHCPV6_STATEFUL)
        # For an IPv6 DHCP stateful subnet, a call to the plugin's
        # _generate_ip method is expected upon port creation.
        try:
            orig_gen_ip = manager.NeutronManager.get_plugin()._generate_ip
            mock_gen_ip = mock.patch.object(
                db_base_plugin_v2.NeutronDbPluginV2,
                '_generate_ip').start()
            mock_gen_ip.side_effect = orig_gen_ip
        ...
        port = self.deserialize(self.fmt, res)
        if mock_gen_ip:
            self.assertTrue(mock_gen_ip.called)
        self.assertEqual(1, len(port['port']['fixed_ips']))
