...
class TunnelAllocationsTest(unittest2.TestCase):
    def setUp(self):
        n1kv_db_v2.initialize()
        n1kv_db_v2.sync_tunnel_allocations(TUNNEL_RANGES)
        self.session = db.get_session()
    ...
    def test_sync_tunnel_allocations(self):
        self.assertIsNone(n1kv_db_v2.get_tunnel_allocation(TUN_MIN - 1))
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MIN).allocated)
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MIN + 1).
                         allocated)
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MAX - 1).
                         allocated)
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MAX).allocated)
        self.assertIsNone(n1kv_db_v2.get_tunnel_allocation(TUN_MAX + 1))

        n1kv_db_v2.sync_tunnel_allocations(UPDATED_TUNNEL_RANGES)

        self.assertIsNone(n1kv_db_v2.get_tunnel_allocation(TUN_MIN + 5 - 1))
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MIN + 5).
                         allocated)
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MIN + 5 + 1).
                         allocated)
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MAX + 5 - 1).
                         allocated)
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(TUN_MAX + 5).
                         allocated)
        self.assertIsNone(n1kv_db_v2.get_tunnel_allocation(TUN_MAX + 5 + 1))

    def test_tunnel_pool(self):
        tunnel_ids = set()
        for x in xrange(TUN_MIN, TUN_MAX + 1):
            tunnel_id = n1kv_db_v2.reserve_tunnel(self.session)
        ...
        n1kv_db_v2.release_tunnel(self.session, tunnel_ids.pop(), TUNNEL_RANGES)
        tunnel_id = n1kv_db_v2.reserve_tunnel(self.session)
        self.assertGreaterEqual(tunnel_id, TUN_MIN)
        self.assertLessEqual(tunnel_id, TUN_MAX)
        tunnel_ids.add(tunnel_id)
        ...
        for tunnel_id in tunnel_ids:
            n1kv_db_v2.release_tunnel(self.session, tunnel_id, TUNNEL_RANGES)

    def test_specific_tunnel_inside_pool(self):
        tunnel_id = TUN_MIN + 5
        self.assertFalse(n1kv_db_v2.get_tunnel_allocation(tunnel_id).allocated)
        n1kv_db_v2.reserve_specific_tunnel(self.session, tunnel_id)
        self.assertTrue(n1kv_db_v2.get_tunnel_allocation(tunnel_id).allocated)
    ...
    def test_specific_tunnel_outside_pool(self):
        tunnel_id = TUN_MAX + 5
        self.assertIsNone(n1kv_db_v2.get_tunnel_allocation(tunnel_id))
        n1kv_db_v2.reserve_specific_tunnel(self.session, tunnel_id)
        self.assertTrue(n1kv_db_v2.get_tunnel_allocation(tunnel_id).allocated)
