...
class IptablesFirewallTestCase(base.BaseTestCase):
    ...
    def test_prepare_port_filter_with_no_sg(self):
        ...
        calls = [call.add_chain('sg-fallback'),
                 ...
                 call.add_rule('ifake_dev', '-d 10.0.0.0/8 -j RETURN'),
                 call.add_rule(
                 ...
                 call.add_rule(
                     'ofake_dev',
                     '-p udp --sport 68 --dport 67 -j RETURN'),
                 call.add_rule('ofake_dev', '-s 10.0.0.0/8 -j RETURN'),
    ...
    def _test_prepare_port_filter(self,
        ...
        calls += [call.add_rule('ifake_dev', '-j $sg-fallback'),
                  ...
                  dhcp_rule]
        if ethertype == 'IPv4':
            calls += [call.add_rule('ofake_dev', '-s 10.0.0.0/8 -j RETURN')]
        calls.append(call.add_rule('ofake_dev', '! -s %s -j DROP' % prefix))
