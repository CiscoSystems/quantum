...
class CSR1kvOVSQuantumAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin):
    ...
    # Bob - Patch to handle trunk ports.
    # This is the link/bridge topology realizing the trunking:
    #  [VM]<--->[Linux bridge]<--->[trunk OVS bridge]<--->[br-int OVS bridge]
    #                       veth-pair               veth-pair
    # I.e., an extra OVS bridge is added so that flow rules to map
    # from host local VLAN tags to link local VLAN tags.
    # To get this topology the
...
#            self.tun_br.delete_flows(dl_dst=port.vif_mac)
        ...
        for net_id, info in trunked_networks.iteritems():
            if net_id not in self.local_vlan_map:
                self.provision_local_vlan(net_id, info['network_type'],
                                          info['physical_network'],
                                          info['segmentation_id'])
            ...
            trunk_bridge.add_flow(
                ...
                dl_vlan=lvm.vlan,
                actions="mod_vlan_vid:%s,normal" % info['vlan'])
            # Ensure incoming traffic from other hosts is forwarded
            if info['network_type'] == constants.TYPE_GRE:
                if self.enable_tunneling:
                    # inbound unicast
                    self.tun_br.add_flow(priority=3,
                                         tun_id=info['segmentation_id'],
                                         dl_dst=port.vif_mac,
    ...
    def port_unbound(self, vif_id, net_uuid=None):
        ...
        # Bob - Patch to handle trunk ports.
        self._process_trunked_networks_remove(vif_id)
        # Bob - End of patch
        ...
        for device in devices:
            ...
            if 'port_id' in details:
                ...
                self.treat_vif_port(port, details['port_id'],
                                    ...
                                    # Bob - Patch to handle trunk ports.
                                    details[trunkport.TRUNKED_NETWORKS])
                                    # Bob - End of patch
            else:
