...
from quantum.plugins.csr1kv_openvswitch import csr1kv_ovs_models_v2
...
def get_network_binding(session, network_id):
    ...
    try:
        binding = (session.query(csr1kv_ovs_models_v2.NetworkBinding).
...
def add_network_binding(session, network_id, network_type,
    ...
    with session.begin(subtransactions=True):
        binding = csr1kv_ovs_models_v2.NetworkBinding(network_id, network_type,
                                                      physical_network,
                                                      segmentation_id)
        session.add(binding)
...
def sync_vlan_allocations(network_vlan_ranges):
    ...
    with session.begin():
        ...
        allocations = dict()
        allocs = (session.query(csr1kv_ovs_models_v2.VlanAllocation).all())
        for alloc in allocs:
        ...
        for physical_network, vlan_ranges in network_vlan_ranges.iteritems():
            ...
            for vlan_id in sorted(vlan_ids):
                alloc = csr1kv_ovs_models_v2.VlanAllocation(physical_network,
                                                            vlan_id)
                session.add(alloc)
...
def get_vlan_allocation(physical_network, vlan_id):
    session = db.get_session()
    try:
        alloc = (session.query(csr1kv_ovs_models_v2.VlanAllocation).
...
def sync_tunnel_allocations(tunnel_id_ranges):
    ...
    with session.begin():
        ...
        allocs = (session.query(csr1kv_ovs_models_v2.TunnelAllocation).
                  all())
        for alloc in allocs:
            try:
        ...
        for tunnel_id in sorted(tunnel_ids):
            alloc = csr1kv_ovs_models_v2.TunnelAllocation(tunnel_id)
            session.add(alloc)
...
def get_tunnel_allocation(tunnel_id):
    session = db.get_session()
    try:
        alloc = (session.query(csr1kv_ovs_models_v2.TunnelAllocation).
...
def get_tunnel_endpoints():
    session = db.get_session()
    try:
        tunnels = session.query(csr1kv_ovs_models_v2.TunnelEndpoint).all()
    except exc.NoResultFound:
        return []
    return [{'id': tunnel.id,
...
def add_tunnel_endpoint(ip):
    session = db.get_session()
    try:
        tunnel = (session.query(csr1kv_ovs_models_v2.TunnelEndpoint).
                  filter_by(ip_address=ip).with_lockmode('update').one())
    except exc.NoResultFound:
        id = _generate_tunnel_id(session)
        tunnel = csr1kv_ovs_models_v2.TunnelEndpoint(ip, id)
        session.add(tunnel)
        session.flush()
    return tunnel
