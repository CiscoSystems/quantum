...
class NvpTests(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for tenant, net, port in self.ports:
            self.quantum.delete_port(self.context, tenant, net, port)
        for tenant, net in self.nets:
            self.quantum.delete_network(self.context, tenant, net)
    ...
    def test_create_multi_networks(self):
        ...
        resp = self.quantum.create_custom_network(self.context,
            "quantum-test-tenant", "quantum-Private-TenantA",
            self.BRIDGE_TZ_UUID, self.quantum.controller)
        resp1 = self.quantum.create_network(self.context,
                                            "quantum-test-tenant",
                                            "quantum-Private-TenantB")
        resp2 = self.quantum.create_network(self.context,
                                            "quantum-test-tenant",
                                            "quantum-Private-TenantC")
        resp3 = self.quantum.create_network(self.context,
                                            "quantum-test-tenant",
                                            "quantum-Private-TenantD")
        net_id = resp["net-id"]

        resp = self.quantum.create_port(self.context,"quantum-test-tenant", net_id,
                                        "ACTIVE")
        port_id1 = resp["port-id"]
        resp = self.quantum.get_port_details(self.context,"quantum-test-tenant", net_id,
                                             port_id1)
        old_vic = resp["attachment"]
        self.assertTrue(old_vic == "None")

        self.quantum.plug_interface(self.context,"quantum-test-tenant", net_id, port_id1,
        ...
        self.quantum.plug_interface(self.context,"quantum-test-tenant", net_id, port_id2,
        ...
        resp = self.quantum.get_all_ports(self.context,"quantum-test-tenant", net_id)

        resp = self.quantum.get_network_details(self.context,"quantum-test-tenant", net_id)

        resp = self.quantum.get_all_networks(self.context,"quantum-test-tenant")

        resp = self.quantum.delete_port(self.context,"quantum-test-tenant", net_id,
                                        port_id1)
        resp = self.quantum.delete_port(self.context,"quantum-test-tenant", net_id,
                                        port_id2)
        self.quantum.delete_network(self.context,"quantum-test-tenant", net_id)
        self.quantum.delete_network(self.context,"quantum-test-tenant", resp1["net-id"])
        self.quantum.delete_network(self.context,"quantum-test-tenant", resp2["net-id"])
        self.quantum.delete_network(self.context,"quantum-test-tenant", resp3["net-id"])

    def test_update_network(self):
        resp = self.quantum.create_network(self.context,"quantum-test-tenant",
            "quantum-Private-TenantA")
        net_id = resp["net-id"]
        try:
            resp = self.quantum.update_network(self.context,"quantum-test-tenant", net_id,
                                               name="new-name")
    ...
    def test_negative_delete_networks(self):
        try:
            self.quantum.delete_network(self.context,"quantum-test-tenant", "xxx-no-net-id")
        except exception.NetworkNotFound:
            self.assertTrue(True)

    def test_negative_get_network_details(self):
        try:
            self.quantum.get_network_details(self.context,"quantum-test-tenant",
                                             "xxx-no-net-id")
        except exception.NetworkNotFound:
            self.assertTrue(True)
    ...
    def test_negative_update_network(self):
        try:
            self.quantum.update_network(self.context,"quantum-test-tenant", "xxx-no-net-id",
                                        name="new-name")
        except exception.NetworkNotFound:
            self.assertTrue(True)

    def test_get_all_networks(self):
        networks = self.quantum.get_all_networks(self.context,"quantum-test-tenant")
        ...
        networks = self.quantum.get_all_networks(self.context,unique_tid)
